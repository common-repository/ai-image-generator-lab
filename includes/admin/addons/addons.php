...
class WPZ_Ai_Image_Lab_Addons {
    ...
    static function setup() {
        add_action( 'admin_enqueue_scripts', [ 'WPZ_Ai_Image_Lab_Addons', 'admin_scripts' ], 11 );
    }
    ...
    public static function admin_scripts($hook) {
        if ( 'wpz-payment_page_wpz-ai-image-lab-settings' === $hook ) {
            wp_enqueue_style( 'ai-image-lab-admin-addons', plugins_url( '/assets/css/admin.min.css', __FILE__ ), [], WpzAiImageLab::VERSION );
        }
    }
    ...
    static function outputList() {
        ...
        if ( empty( $addons ) ) {
            echo(
                ...
                . esc_html__( 'We are unable to load the addons for this theme right now. Please try again later!', 'wpz-ai-image-lab' )
                . '</p>'
            );
        ...
        switch ( $addon->type ) {
            ...
            case 'repo':
                ...
                if ( empty( $installed_plugin ) ) {
                    $label = __( 'Install', 'wpz-ai-image-lab' );
                    if ( current_user_can( 'install_plugins' ) ) {
                        $url             = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=' . $addon->target ), 'install-plugin_' . $addon->target );
                        $onclick_escaped = '';
                    } else {
                        $url             = '#';
                        $onclick_escaped = ' onclick="alert(\'' . esc_js( __( 'You do not have sufficient permissions to install plugins. If you think this message is in error, go to Plugins > Add New and search for the plugin name.', 'wpz-ai-image-lab' ) ) . '\');return false;"';
                    }
                } else {
                    $label           = __( 'Already Installed', 'wpz-ai-image-lab' );
